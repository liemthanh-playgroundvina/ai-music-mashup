...
def crossfade(master_song, slave_song, len_crossfade, len_time_stretch, return_audio=True, large_master=True):
    ...
    if large_master:
        volume_master = [1.0, 0.2]
        volume_slave = [0.0, 0.2]
    else:
        volume_master = [0.6, 0.0]
        volume_slave = [0.1, 0.9]

    # Sound Effects for Master
    new_master_fadedout = linear_fade_volume(master_fadeout_audio, start_volume=volume_master[0], end_volume=volume_master[1])
    new_master_fadedout = linear_fade_filter(new_master_fadedout, 'low_shelf', start_volume=volume_master[0], end_volume=volume_master[1])
    new_master_fadedout = linear_fade_filter(new_master_fadedout, 'high_shelf', start_volume=volume_master[0], end_volume=volume_master[1])

    # Sound Effects for Slave
    new_slave_fadedin = linear_fade_volume(slave_fadein_audio, start_volume=volume_slave[0], end_volume=volume_slave[1])
    new_slave_fadedin = linear_fade_filter(new_slave_fadedin, 'low_shelf', start_volume=volume_slave[0], end_volume=volume_slave[1])
    new_slave_fadedin = linear_fade_filter(new_slave_fadedin, 'high_shelf', start_volume=volume_slave[0], end_volume=volume_slave[1])
